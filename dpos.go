...
func NewDPOSService(cfg *Config) *DPOSService {
	pow := DPOSService{
		Cfg:          cfg,
		started:      false,
		manualMining: false,
...
func (pow *DPOSService) SetMinerAddr(minerAddr string) {
	pow.Cfg.MinerAddr = minerAddr
}

func (pow *DPOSService) GetTransactionCount() int {
	transactionsPool := pow.Cfg.TxMemPool.GetTxsInPool()
	return len(transactionsPool)
}

func (pow *DPOSService) CollectTransactions(msgBlock *types.Block) int {
	txs := 0
	transactionsPool := pow.Cfg.TxMemPool.GetTxsInPool()
...
func (pow *DPOSService) DiscreteMining(n uint32) ([]*common.Uint256, error) {
	pow.Mutex.Lock()
...
func (pow *DPOSService) SolveBlock(msgBlock *types.Block, ticker *time.Ticker) bool {
...
func (pow *DPOSService) BroadcastBlock(block *types.Block) {
	pow.Cfg.Server.BroadcastMessage(msg.NewBlock(block))
}

func (pow *DPOSService) Start() {
	pow.Mutex.Lock()
...
func (pow *DPOSService) CpuMining() {
	ticker := time.NewTicker(time.Second * hashUpdateSecs)
	defer ticker.Stop()
